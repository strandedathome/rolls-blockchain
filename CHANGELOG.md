...
- Beta builds are built on every merge to main, and are now available from <https://pecanrolls.net/download/>.
...
- Added a py.typed file that allows other projects that pip install rolls-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the rolls install and then converts it into proper npm format and puts that into package.json.
...
- Switched IP address lookup to first use PecanRolls's service ip.pecanrolls.net.
...
- Thanks @mishan for your change to 'rolls plotnft show' to display Percent Successful Points.
...
- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `rolls plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/strandedathome/rolls-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/strandedathome/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `rolls farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `rolls configure` now supports command line updates to peer count and target peer count.
...
- Correctly display private key in `rolls keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `rolls keys show`.
...
- We added a simple profiler to track performance times in the application (see /rolls/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `rolls configure -t true|false`).
...
- Secret wallet key is hidden by default in `rolls keys show`. Use `rolls keys show --show-mnemonic-seed` for private keys.
...
- You should review the [release notes for v1.0.2](https://github.com/strandedathome/rolls-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- We have released version 1.0.0 of [chiapos](https://github.com/strandedathome/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/rolls-blockchain).
...
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/rolls-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/rolls-blockchain).
...
- This is a hard fork/breaking change from RC6/7. TROLLS Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.pecanrolls.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `rolls farm summary` will now display TROLLS or ROLLS as appropriate.
...
- `rolls farm summary` displayed some incorrect amounts.
...
- `rolls netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `rolls plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `rolls keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We have moved to a new and much higher scalability download.pecanrolls.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.pecanrolls.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.pecanrolls.net/latest/Setup-MacOS.dmg).
- PecanRolls wheels not on Pypi and some dependecies not found there also are now on pypi.pecanrolls.net.
...
- The Farm page will now no longer get stuck at 50 TROLLS farmed.
- `rolls farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `rolls wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `rolls_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TROLLS will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.pecanrolls.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `rolls farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/rolls-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `rolls wallet` now takes get_address to get a new wallet receive address from the CLI.
- `rolls plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of rolls-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in rolls-blockchain.
...
- Adding Click changed the behavior of `rolls keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.pecanrolls.net via https, for example.
...
- RC3 is a new chain to support the last major chialisp changes. TROLLS from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/rolls-blockchain).
- You can obtain a new wallet receive address on the command line with `rolls wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/strandedathome/rolls-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `rolls -h` and `rolls SUBCOMMAND -h` can be your guide.
...
- `rolls plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `rolls netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `rolls init` only migrates release candidate directories. The versioned sub directories under `~/rolls` will be going away before mainnet.
...
- `rolls configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/strandedathome/rolls-blockchain/releases/tag/1.0rc1) was well.
...
- Added `rolls plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'rolls wallet send' now takes in TROLLS or ROLLS as units instead of bytes.
...
- The Beta 27 chain is a hard fork. All TROLLS from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `rolls wallet -h` to learn more. Also, `rolls wallet` now requires a third argument of `show`, therefor you will use `rolls wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [rolls blockchain gui](https://crowdin.com/project/rolls-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `rolls init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `rolls init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TROLLS/ROLLS per block for a total of 9216 TROLLS per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from rolls-blockchain, [rolls-blockchain-gui](https://github.com/strandedathome/rolls-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `rolls-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `rolls wallet`, was incorrectly displaying TROLLS balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/strandedathome/rolls-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/strandedathome/rolls-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/strandedathome/rolls-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `rolls plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- `rolls configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `rolls plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `rolls plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `rolls show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `rolls show` handles sub block 0 better.
...
- `rolls plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `rolls plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `rolls show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `rolls configure --enable-upnp`.
...
- `rolls configure` now lets you enable or disable uPnP.
...
- `rolls netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `rolls show` of the GUI use `rolls netspace -d 1000` as `rolls netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TROLLS. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TROLLS compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TROLLS.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [pecanrolls.net](https://pecanrolls.net/). Among the improvements this gives the PecanRolls blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TROLLS from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TROLLS to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TROLLS to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `rolls plots create`. After obtaining the fingerprint from `rolls keys show`, try `rolls plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `rolls configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `rolls configure -h`. Note that if rolls services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TROLLS you've won so far. Harvester and Full Node connections have moved to Advanced Options.
...
- `rolls show -s` has been refactored to support the new consensus.
- `rolls netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.rolls/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run rolls-blockchain.
...
- `rolls show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/strandedathome/rolls-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install rolls-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/strandedathome/rolls-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/strandedathome/rolls-blockchain/issues/485) where log level could no longer be set by service.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/strandedathome/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all rolls services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test rolls since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve rolls-blockrollsn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from PecanRolls Network at backup.pecanrolls.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the PecanRolls backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.pecanrolls.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test rolls are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.rolls` directory.
...
- `rolls init` now automatically discovers previous releases in each new release.

### Fixed

- `rolls show -w` should now more reliably work. Wallet balances should be more often correct.
...
- `rolls show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- See wallet balances in command line: `rolls show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `rolls keys sign` and `rolls keys verify` to allow farmers to certify their ownership of keys.
...
- `rolls start wallet-server` changed to `rolls start wallet`, for consistency.
...
- Removed legacy scripts such as rolls-stop-server, rolls-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/strandedathome/bls-signatures) to use the
...
- Changed ROLLS units to TROLLS units for testnet.
...
"rolls plots create" and "rolls plots check".
- Add plot directories to config.yaml from the cli with "rolls plots add".
...
- Starting rolls now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'rolls keys' command line.
...
- We added a `rolls keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the rolls-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `rolls netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `rolls-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `rolls-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `rolls-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `rolls-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `rolls init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `rolls start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `rolls-start-harvester`.
...
- The user interface now refers to rolls coins with their correct currency code of ROLLS.
...
- Now you can run estimates of the total space currently farming the network. Try `rolls netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during rolls init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `rolls show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `rolls show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/strandedathome/rolls-blockchain/wiki) and placed in [INSTALL](https://github.com/strandedathome/rolls-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/strandedathome/rolls-blockchain/wiki/Quick-Start-Guide)
...
- The script `rolls start wallet-gui` has been chaned to `rolls start wallet` which launches but the GUI and server on MacOS and Linux. `rolls start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/strandedathome/rolls-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `rolls stop all` command sometimes fails, use `rolls-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `rolls show`.

### Changed

- `rolls-restart-harvester` has been renamed from `rolls-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.rolls/beta-1.0b3/config/plots.yaml` after migrating
...
- Added color logging
- Now rolls_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to rolls_full node, rolls_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new rolls services were started.
...
- More details on the release at [https://www.pecanrolls.net/developer/](https://www.pecanrolls.net/developer/)

[unreleased]: https://github.com/strandedathome/rolls-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/strandedathome/rolls-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/strandedathome/rolls-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/strandedathome/rolls-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/strandedathome/rolls-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/strandedathome/rolls-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/strandedathome/rolls-blockchain/releases/tag/Alpha-1.0
