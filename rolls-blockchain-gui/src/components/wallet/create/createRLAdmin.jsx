...
import { AlertDialog } from '@rolls/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      rollsper_input.value === '' ||
      Number(rollsper_input.value) === 0 ||
      !Number(rollsper_input.value) ||
      isNaN(Number(rollsper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
