...
import {
  ...
  Fee,
  Form,
  TextField as PecanRollsTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of rolls in the blockchain at the current
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send rolls to coloured address. Please enter a rolls
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    if (address.slice(0, 12) === 'rolls_addr://') {
    ...
    const amountValue = Number.parseFloat(rolls_to_pecan(amount));
    const feeValue = Number.parseFloat(rolls_to_pecan(fee));
