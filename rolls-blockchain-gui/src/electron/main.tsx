...
import rollsEnvironment from '../util/rollsEnvironment';
import rollsConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!rollsEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    rollsConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!rollsConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !rollsConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (rollsConfig.manageDaemonLifetime()) {
        rollsEnvironment.startPecanRollsDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'PecanRolls Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/strandedathome/rolls-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/strandedathome/rolls-blockchain/wiki/FAQ',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/strandedathome/rolls-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          //{
          ...
          //},
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/strandedathome/rolls-blockchain/issues',
              );
            },
          },
          //{
		  ...
		  {
            label: i18n._(/* i18n */ { id: 'Visit PecanRolls Website' }),
            click: () => {
              openExternal(
                'https://pecanrolls.net',
              );
            },
          },
		  {
          ...
          },	
          {
            ...
            click: () => {
              openExternal(
                'https://www.facebook.com/PecanRollsNetwork',
              );
            },
          },	
          {
          ...
          },	
          {
            ...
            click: () => {
              openExternal(
                'https://www.reddit.com/r/PecanRollsNetwork',
              );
            },
          },
        ],
    ...
    if (process.platform === 'darwin') {
      // PecanRolls Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'PecanRolls' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About PecanRolls Blockchain' }),
            click: () => {
              openAbout();
            },
          },
