...
from rolls.cmds.init_funcs import check_keys, rolls_init
from rolls.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from rolls.daemon.keychain_server import KeychainServer, keychain_commands
from rolls.daemon.windows_signal import kill
from rolls.server.server import ssl_context_for_root, ssl_context_for_server
from rolls.ssl.create_ssl import get_mozilla_ca_crt
from rolls.util.rolls_logging import initialize_logging
from rolls.util.config import load_config
from rolls.util.json_util import dict_to_json_str
...
from rolls.util.service_groups import validate_service
from rolls.util.setproctitle import setproctitle
from rolls.util.ws_message import WsRpcMessage, create_payload, format_response
...
service_plotter = "rolls plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "rolls": "rolls",
        "rolls_wallet": "start_wallet",
        "rolls_full_node": "start_full_node",
        "rolls_harvester": "start_harvester",
        "rolls_farmer": "start_farmer",
        "rolls_introducer": "start_introducer",
        "rolls_timelord": "start_timelord",
        "rolls_timelord_launcher": "timelord_launcher",
        "rolls_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("rolls daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "rolls_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in rolls_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    rolls_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("rolls_daemon")
