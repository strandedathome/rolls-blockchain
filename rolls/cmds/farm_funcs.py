...
from rolls.cmds.units import units
...
from rolls.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total rolls farmed: {amounts['farmed_amount'] / units['rolls']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['rolls']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['rolls']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'rolls start wallet' and 'rolls wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'rolls wallet show'")
    else:
        print("Note: log into your key using 'rolls wallet show' to see rewards for each key")
