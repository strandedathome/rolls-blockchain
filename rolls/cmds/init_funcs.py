...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "rolls_target_address" not in config["farmer"]:
        print(
            f"setting the rolls destination for the 100% farmer reward to {all_targets[0]}"
        )
    ...
    if "rolls_target_address" not in config["pool"]:
        print(f"setting the rolls destination address for no pool reward to {all_targets[0]}")
