...
from rolls.daemon.server import singleton, service_launch_lock_path
from rolls.server.ssl_context import rolls_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"rolls_{service_name}"
        ...
        rolls_ca_crt, rolls_ca_key = rolls_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = PecanRollsServer(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for PecanRollsServer to be closed")
