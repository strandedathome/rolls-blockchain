...
from rolls.protocols.protocol_state_machine import message_requires_reply
from rolls.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
...
from rolls.util.network import is_localhost, is_in_network
from rolls.util.ssl import verify_ssl_certs_and_keys
...
class PecanRollsServer:
    def __init__(
        ...
        rolls_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.rolls_ca_crt_path, self.rolls_ca_key_path = rolls_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.rolls_ca_crt_path, self.rolls_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use rolls's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.pecanrolls.net/") as resp:
                    if resp.status == 200:
