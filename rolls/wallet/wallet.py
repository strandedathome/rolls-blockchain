...
from rolls.wallet.puzzles.puzzle_utils import (
...
from rolls.wallet.secret_key_store import SecretKeyStore
from rolls.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_rolls(self, rolls_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if rolls_amount < 0:
            utxos = await self.select_coins(abs(rolls_amount), exclude)
        else:
        ...
        rolls_amount = spend_value + rolls_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": rolls_amount}]
